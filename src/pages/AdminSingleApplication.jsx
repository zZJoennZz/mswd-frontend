...
const AdminSingleApplication = () => {
    ...
    const onChangeText = (e) => setUpdated({...updated, [e.target.name] : e.target.value})
    ...
    const submitTrack = async (e) => {
        ...
        await apifrm.post(`application/status/post`, frmData)
            .then(res => {
    ...
    return (
        <Container fluid>
            ...
            <Row className="mb-3">
                                ...
                                <ListGroup>
                                    ...
                                    >
                                        <div className="ms-2 me-auto">
                                            <div className="fw-bold">{appliStat.length > 0  ? <span>Submitted</span> : <span>Error</span>}</div>
                                            {appliStat.length > 0  ? appliStat[0].statusMsg : <span>Error</span>}
                                        </div>
                                    </ListGroup.Item>
                                    <ListGroup.Item
                                        as="li"
                                    ...
                                    >
                                        <div className="ms-2 me-auto">
                                            <div className="fw-bold">{appliStat.length > 0  ? <span>Processing</span> : <span></span>}</div>
                                            {appliStat.length > 1  ? appliStat[1].statusMsg : <span>...</span>}
                                        </div>
                                    </ListGroup.Item>
                                    <ListGroup.Item
                                        as="li"
                                    ...
                                    >
                                        <div className="ms-2 me-auto">
                                            <div className="fw-bold">{appliStat.length > 0  ? <span>Result</span> : <span></span>}</div>
                                            {appliStat.length > 2 ? appliStat[2].status === 3 ? <span>Approved: {appliStat[2].statusMsg}</span> : <span>Denied {appliStat[2].statusMsg}</span> : <span>...</span>}
                                        </div>
                                    </ListGroup.Item>
                                </ListGroup>
                        }
                ...
                <Col md={6}>
                    {
                        appliStat && appliStat.length === 3 ? appliStat[1].status === 2 ? "Denied" : "Completed" :
                            <Form onSubmit={submitTrack}>
                                <Form.Control onChange={onChangeText} className="mb-2" type="text" placeholder="Enter notes to client" name="statusMsg" id="statusMsg" />
                                { 
                                    appliStat.length === 2 && appliStat[1].status === 1 ? 
                                        <Form.Select className="mb-2" onChange={onChangeText} name="status" id="status">
                                ...
                                {!appliStat ? <Button type="submit" disabled>Loading...</Button> : <Button type="submit">{ appliStat.length === 1 ? "Start Processing" : "Send Result" }</Button>}
                            </Form>
                    }
                </Col>
            </Row>
