...
const AdminSettings = ({ userId }) => {
    ...
    const getUserList = async () => {
        try {
            ...
            setShowToast(true);
            setToastMsg("Something went wrong. User not fetched. Please refresh the page or contact website administrator.")
        }
    }
    ...
    const onSubmitSaveUser = async (frmDat, userId) => {
        ...
        try {
            await api.put(`updatesingle/${userId}`, data)
            .then(res => {
                setShowToast(true);
            ...
            .catch(err => {
                setShowToast(true);
                setToastMsg("Something went wrong. User changes is not saved. Please refresh the page or contact website administrator.");
            })
        } catch (error) {
            setShowToast(true);
            setToastMsg("Something went wrong. User changes is not saved. Please refresh the page or contact website administrator.");
    ...
    const onSubmitAddNew = async (e) => {
        ...
        await apifrm.post('signup', frmData)
            ...
            .catch(e => {
                setShowToast(true);
                setToastMsg("Something went wrong. User is not saved. Please refresh the page or contact website administrator.")
            });
    }
    ...
    const onSubmitSave = async (frmData) => {
        try {
            ...
            await api.put('updateprofile', data)
                .then(res => {
                    setShowToast(true);
    ...
    const onChangeText = (e) => setUserDetail({...userDetail, [e.target.name] : e.target.value})
    ...
    return (
        <Container fluid>
            ...
            <Row>
                <Col md={12}>
                    <Tab.Container id="left-tabs" defaultActiveKey={ userId !== 1 ? "manage" : "add" }>
                        <Row>
                            <Col sm={3}>
                                ...
                                <Tab.Content>
                                    {
                                        ...
                                        <Tab.Pane eventKey="add">
                                            <AdminAddUser userDets={userDetail} onSubmitAdd={onSubmitAddNew} onChangeField={onChangeText} />
                                        </Tab.Pane>
                                    }
                                    ...
                                    </Tab.Pane>
                                    {
                                        ...
                                        <Tab.Pane eventKey="manage-user">
                                            <AdminUserList saveChanges={onSubmitSaveUser} usrList={userList} />
                                        </Tab.Pane>
                                    }
                                </Tab.Content>
                            </Col>
            ...
            <GlobalToast onClose={() => setShowToast(false)} show={showToast} msg={toastMsg} title="Notification" />
        </Container>
    )
}
