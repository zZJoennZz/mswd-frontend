...
const AdminSingleInquiry = () => {
    ...
    const saveStaffNote = async (e) => {
        try {
            if (staffNote.trim() !== "") { 
                await api.put(`client_message/put_note/${inqId}`, {notes : staffNote})
    ...
    return (
        <Container fluid>
            <Breadcrumb>
                ...
                <Breadcrumb.Item active>{!inq ? <><Spinner animation="border" size="sm" /> Fetching data...</> : <>{inq.subject} by {inq.full_name}</>}</Breadcrumb.Item>
            </Breadcrumb>
            <Row>
                <Col md={12}>
                    {
                            ...
                            <Table responsive striped bordered hover>
                                ...
                                <tbody>
                                    <tr>
                                        ...
                                        <td>
                                            <a href={"mailto:" + inq.email_address}>{inq.email_address}</a>
                                        </td>
                                    </tr>
                                    <tr>
                                        <th>
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <Form.Control className="mb-2" as="textarea" defaultValue={inq.notes} onChange={noteOnChange} />
                                            <Button onClick={saveStaffNote}>Save Notes</Button>
                                        </td>
                                    </tr>
                                    <tr>
                                        <th>
                                            Status <small className="text-muted">Click to change status</small>
                                        </th>
                                        <td>
                                        <p style={{ cursor: "pointer" }} onClick={changeStatus}>{inq.status === "0" ? <Badge bg="warning">Unsolve</Badge> : <Badge bg="success">Solved</Badge>}</p>
                                        </td>
                                    </tr>
                                </tbody>
                            </Table>
