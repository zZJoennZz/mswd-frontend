...
const AdminOrgChart = () => {
  ...
  const onSaveBtn = () => {
    ...
    if (frmBtnText === "Save Entry") {
      try {
        axios
          .post(`${process.env.REACT_APP_API}org/chart/post`, formData, {
            headers: headers,
          })
          .then((res) => {
      ...
      try {
        axios
          .put(`${process.env.REACT_APP_API}org/chart/put/${orgId}`, formData, {
            headers: headers,
          })
          .then((res) => {
            setToastMsg("Entry is saved");
  ...
  React.useEffect(() => {
    const getOrgChart = async () => {
      await axios
        .get(`${process.env.REACT_APP_API}org`)
        ...
        <Table responsive striped bordered hover>
          ...
          <tbody>
            {orgChart.map((d) => (
              <tr key={d.id}>
                <td
                  onClick={openModal.bind(this, "edit", d)}
...
const NewOrgChart = ({ textOnChange }) => {
  ...
  React.useEffect(() => {
    ...
    const getPerson = async () => {
      await axios
        .get(`${process.env.REACT_APP_API}org/person`, { headers: headers })
        .then((res) => isActive && setPersonList(res.data.data));
    };
    const getPosition = async () => {
      await axios
        .get(`${process.env.REACT_APP_API}org/position`, { headers: headers })
        .then((res) => isActive && setPositionList(res.data.data));
    };
    const getDivision = async () => {
      await axios
        .get(`${process.env.REACT_APP_API}org/division`, { headers: headers })
  ...
  return !divisionList || !personList || !positionList ? (
    "Loading..."
  ) : (
    <Form>
      <Row>
...
const EditOrgChart = ({ textOnChange, data }) => {
  ...
  React.useEffect(() => {
    ...
    const getOrgId = async () => {
      await axios
        .get(`${process.env.REACT_APP_API}org/chart/${data.id}`, {
          headers: headers,
        })
        .then((res) => {
        ...
        <Col md={12}>
          <Form.Group className="mb-3">
            ...
            <Form.Select
              ...
              id="position_id"
              onChange={textOnChange}
              defaultValue={orgId.position_id}
            >
              <option disabled>Select</option>
              {!positionList
                ? "Loading..."
        ...
        <Col md={12}>
          <Form.Group>
            ...
            <Form.Select
              ...
              id="division_id"
              onChange={textOnChange}
              defaultValue={orgId.division_id}
            >
              <option disabled>Select</option>
              {!divisionList
                ? "Loading..."
