...
const AdminOrgPeople = () => {
  ...
  let [perImg, setPerImg] = React.useState(0);
  const fileOnChange = (e) => {
    setPerImg(e.target.files);
  };
  ...
  const onSaveBtn = async () => {
    ...
    data.append("middle_initial", mInitial);
    data.append("last_name", lName);
    data.append("suffix", nSuffix);
    data.append("gender", gender);
    data.append("birthday", birthday);
    data.append("img_path", perImg[0]);

    if (frmTitle === "New Person") {
      try {
        await axios
          .post(`${process.env.REACT_APP_API}org/person/post`, data, {
            headers: headers,
          })
          .then((res) => {
      ...
      try {
        await axios
          .post(`${process.env.REACT_APP_API}org/person/put/${editId}`, data, {
            headers: headers,
          })
          .then((res) => {
  ...
  const delPerson = async (perId) => {
    ...
    if (a) {
      try {
        await axios
          .delete(`${process.env.REACT_APP_API}org/person/delete/${perId}`, {
            headers: headers,
          })
          .then((res) => {
            if (res.data.success) {
  ...
  React.useEffect(() => {
    ...
    const getPpl = async () => {
      ...
      try {
        let res = await axios.get(`${process.env.REACT_APP_API}org/person`, {
          headers: headers,
        });
            ...
            {orgPer.map((d) => (
              <tr key={d.id}>
                ...
                <td>
                  ...
                  <Button
                    onClick={delPerson.bind(this, d.id)}
                    style={{ float: "right" }}
                    variant="danger"
                    size="sm"
                  >
...
const NewPerson = ({ textOnChange, fileOnChange }) => {
  return (
    <Form>
      <Row>
        <Col md={4}>
      ...
      <Row>
        ...
        <Col md={12}>
          <Form.Group className="mb-3">
            <Form.Label>Person Image (Upload square photos)</Form.Label>
            <Form.Control
              type="file"
              name="file"
              id="file"
